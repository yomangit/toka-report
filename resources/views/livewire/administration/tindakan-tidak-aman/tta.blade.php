<div>
    ...
    <div class="overflow-x-auto">
        <table class="table table-zebra table-xs">
            ...
            <tbody>
                <!-- row 1 -->
                @forelse ($TTA as $no => $cc)
                <tr class="text-center">
                    <th>{{ $TTA->firstItem() + $no }}</th>
                    <td>{{ $cc->name }}</td>
                    <td>
                        <div class="">
                            ...
                            <label class="btn btn-xs btn-square btn-error tooltip tooltip-top tooltip-error" wire:click="delete({{ $cc->id }})" wire:confirm.prompt="Are you sure delete {{ $cc->name_category_company }}?\n\nType DELETE to confirm|DELETE" data-tip="Delete">
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="size-6">
    ...
    <dialog class="{{ $modal }}">
        <div class="modal-box">
            <div class="py-4 font-extrabold text-transparent divider divider-info bg-clip-text bg-gradient-to-r from-pink-500 to-violet-500">
                {{ $divider }}</div>
